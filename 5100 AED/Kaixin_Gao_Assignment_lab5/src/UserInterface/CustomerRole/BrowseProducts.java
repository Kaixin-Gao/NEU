...
public class BrowseProducts extends javax.swing.JPanel {
    ...
    public void reprotable(String keyword){
        DefaultTableModel model = (DefaultTableModel)productTable.getModel();
        model.setRowCount(0);
        for(Supplier s : supplierDir.getSupplierlist()){
    ...
    private void addtoCartButton6ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_addtoCartButton6ActionPerformed
        ...
        if (selectedRow < 0){
            JOptionPane.showMessageDialog(null, "pls selecet a row");
            return;
        }
        Product selectedProduct = (Product)productTable.getValueAt(selectedRow, 0);
        int fechqty =(Integer) quantitySpinner.getValue();
        if (fechqty < 0 ){
            JOptionPane.showMessageDialog(null, "qty not right");
            return;
        }
        try{
            double salePrice = Double.parseDouble(txtSalesPrice.getText());
            if(salePrice <= selectedProduct.getPrice()){
                JOptionPane.showMessageDialog(null, "SalePrice error");
                return;
            }
            if (fechqty <= selectedProduct.getAvail()){
                boolean alreadyPresent = false;
                for(OrderItem oi : order.getOrderItemList()){
                    if(oi.getProduct() == selectedProduct){
                        int oldAvail = selectedProduct.getAvail();
                        int newAvail = oldAvail - fechqty;
                        selectedProduct.setAvail(newAvail);
                        oi.setQuatity(fechqty+oi.getQuatity());
                        alreadyPresent = true;
                        populateTable();
                        refreshOrderTable();
                        break;
                ...
                if(!alreadyPresent){
                    ...
                    int newAvail = oldAvail - fechqty;
                    selectedProduct.setAvail(newAvail);
                    order.addOrderItem(selectedProduct, fechqty, salePrice);
                    populateTable();
                    refreshOrderTable();
                }
                else{
    ...
    private void btnModifyQuantityActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnModifyQuantityActionPerformed
        ...
        int currentAvail = oi.getQuatity();
        int oldqty = oi.getQuatity();
        int newqty = Integer.parseInt(txtNewQuantity.getText());
        if(newqty >(currentAvail + oldqty)){
             JOptionPane.showMessageDialog(null, "qty more than availible");
             return;
        }
        else{
            if(newqty <= 0){   
                 JOptionPane.showMessageDialog(null, "qty should more than 0");
                 return;
            }
    ...
    private void btnRemoveOrderItemActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnRemoveOrderItemActionPerformed
       ...
       if (row<0){
           JOptionPane.showMessageDialog(null, "select a row pls");
       }
    ...
    private void btnViewOrderItemActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnViewOrderItemActionPerformed
        ...
        OrderItem oi = (OrderItem)orderTable.getValueAt(row, 0);
        ViewOrderItemDetailJPanel voidjp = new ViewOrderItemDetailJPanel(upc, oi);
        upc.add("ViewOrderItemDetailJPanel", voidjp);
        CardLayout layout = (CardLayout) upc.getLayout();
        layout.next(upc);
