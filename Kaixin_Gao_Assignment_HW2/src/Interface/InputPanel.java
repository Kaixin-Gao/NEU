...
public class InputPanel extends javax.swing.JPanel {
    ...
    private void createBtnActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_createBtnActionPerformed
      ...
      if((!"    ".equals(name) && name.length() != 4) && (name.replaceAll("[\u4e00-\u9fa5]*[a-z]*[A-Z]* *", "").length()==0)){
              ...
              if((!"".equals(phone) && phone.length() == 10) && (phone.replaceAll("[0-9]*", "").length()==0)){
                                   ...
                                   if(femaleRB.isSelected()){
                                    v.setSex("Female");
                                       ...
                                       {v.setSex("Male");}else{v.setSex("UNKNOW");}
                                   }
                                   
                                    if(internationalRB.isSelected()){
                                    v.setNation("International");
                                     }else{if(citizenRB.isSelected())
                                       {v.setNation("Citizen");}else{v.setNation("UNKNOW");}}
                                    
                                    if(graduatedRB.isSelected()){
                                    v.setGraduated("Graduated");
                                     }else{if(ungraduatedRB.isSelected())
                                       {v.setGraduated("Ungraduated");}else{v.setGraduated("UNKNOW");}}
        ...
        cppCB.setSelected(false);
        chongCB.setSelected(false);
        jsCB.setSelected(false);
        javaCB.setSelected(false);
        phpCB.setSelected(false);
